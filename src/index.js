...
bot.on('message', async (ctx) => {
  ...
  console.log(`触发AI回复: chat=${chatId}, mode=${trigger}, text=${ctx.message.text.slice(0, 80)}`);
  ...
  try {
    ...
    console.log(`AI决策: chat=${chatId}, mode=${trigger}, shouldReply=${shouldReply}, replyLength=${reply.length}`);
